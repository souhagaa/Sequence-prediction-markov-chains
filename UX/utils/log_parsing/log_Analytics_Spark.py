...
def load_log_file(raw_data_files=None):
...
def load_dataframe():
    ...
    base_df = spark.read.format("csv").option("header", "true").load("/home/souhagaa/Bureau/test/server/UX/UX/data/interm/analytic_log.csv")
    return base_df


def save_spark_df(df, name, output_file_path=None):
    ...
    if output_file_path is None:
        final = '/home/souhagaa/Bureau/test/server/UX/UX/data/interm/{}.csv'.format(name)
    else:
    ...
    file = fs.globStatus(sc._jvm.Path(part))[0].getPath().getName()
    fs.rename(sc._jvm.Path(output1 + '/' + file), sc._jvm.Path(final))
    fs.delete(sc._jvm.Path(output1), True)
    path = '/home/souhagaa/Bureau/test/server/UX/UX/data/interm/.{}.csv.crc'.format(name)
    fs.delete(sc._jvm.Path(path), True)
    print("dataframe saved")


# Data Wrangling
def data_wrangling(base_df, output_file_path=None):
    ...
    df.printSchema()
    df.limit(5).toPandas()
    save_spark_df(logs_df, "analytic_log")
    save_spark_df(df, "parsed_log", output_file_path)
# Let's now cache `logs_df` since we will be using it extensively for our data
# analysis section in the next part!
# logs_df.cache()


def content_size_summary(logs_df):
...
def http_status_analysis(logs_df):
...
def top_twenty_frequent_urls(logs_df):
...
def top_ten_error_endpoints(logs_df):
...
def number_unique_hosts(logs_df):
...
def get_unique_daily_users(logs_df):
...
def number_unique_daily_hosts(logs_df):
    ...
    host_day_distinct_df = get_unique_daily_users(logs_df)
...
def daily_requests_per_host(logs_df):
...
def get_404_responses(logs_df):
    ''' this is an inside function
    '''
    return logs_df.filter(logs_df["status"] == 404)


def get_errors_by_date(logs_df):
    ''' this is an inside function
    '''
    not_found_df = get_404_responses(logs_df)
...
def print_total_404_responses(logs_df):
    ...
    not_found_df = get_404_responses(logs_df).cache()
    print(('Total 404 responses: {}').format(not_found_df.count()))


def top_twenty_404_responses_endpoints(logs_df):
    not_found_df = get_404_responses(logs_df)
...
def top_twenty_404_hosts(logs_df):
    not_found_df = get_404_responses(logs_df)
...
def errors_404_per_day(logs_df):
    ...
    errors_by_date_sorted_df = get_errors_by_date(logs_df)
...
def top_three_days_404_error(logs_df):
    errors_by_date_sorted_df = get_errors_by_date(logs_df)
